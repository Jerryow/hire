...
ChkUtil.isOrderCode = function (str, orderType) {
    ...
    if (_orderType.indexOf('|') > -1) {
        for (var i = 0; i < _orderType.split("|").length; i++) {
            _orderCode = _orderCode
                .replace(_orderType.split("|")[i].toUpperCase(), "")
                .replace(_orderType.split("|")[i].toLowerCase(), "");
        }
    }
    _orderCode = _orderCode
        .replace(_orderType.toUpperCase(), "")
        .replace(_orderType.toLowerCase(), "");
    var patrn = /^\d{1,7}$/;
...
ChkUtil.fileType = function (vName) {
    if (ChkUtil.compare(vName, /\.(gif|png|jpe?g)$/i)) { return "image"; }
    else if (ChkUtil.compare(vName, /\.(htm|html)$/i)) { return "html"; }
    else if (ChkUtil.compare(vName, /\.(rtf|docx?|xlsx?|pptx?|pps|potx?|ods|odt|pages|ai|dxf|ttf|tiff?|wmf|e?ps)$/i)) { return "office"; }
    else if (ChkUtil.compare(vName, /\.(txt|md|csv|nfo|ini|json|php|js|css)$/i)) { return "text"; }
    else if (ChkUtil.compare(vName, /\.(og?|mp4|webm|mp?g|mov|3gp)$/i)) { return "video"; }
    else if (ChkUtil.compare(vName, /\.(og?|mp3|mp?g|wav)$/i)) { return "audio"; }
    else if (ChkUtil.compare(vName, /\.(swf)$/i)) { return "flash"; }
    else if (ChkUtil.compare(vName, /\.(pdf)$/i)) { return "pdf"; }
    else { return "other"; }
}
