var vm = new Vue({
    ...
    methods: {
        ...
        getLetterDetails: function (id) {
            axiosGet(api.account + "GetLetterTemplateDetail?PKID=" + id).then(res => {
                if (res.code == 1) {
        ...
        saveLetterTemplateDate: function () {
            axiosPost(api.account + "SaveLetterTemplate", this.form).then(res => {
                if (res.code == 1) {
        ...
        destroyLetter: function (id) {
            this.$confirm('确认删除?', '提示').then(res => {
                axiosGet(api.account + "DestroyLetter?PKID=" + id).then(res => {
                    if (res.code == 1) {
                        this.$alert('删除成功', '提示');
                        this.getLetterList();
                    }
