var vm = new Vue({
    ...
    methods: {
        ...
        getBasicProfile: function () {
            axiosGet(api.candidate + "GetAllFollowList", this.form).then(res => {
                if (res.data) {
        ...
        getFolderProfile: function () {
            axiosGet(api.candidate + "GetUserFollowList", this.form).then(res => {
                if (res.data != null) {
                    this.contract = res.data.contract;
                    this.contractHelp = res.data.contractHelp;
                    this.profile = res.data.data.DataList;
        ...
        getSingleUserFollow: function (id) {
            axiosGet(api.candidate + "GetSingleUserFollow", { userId: id }).then(res => {
        ...
        editProfile: function () {
            ...
            if (this.userFolders.length == 0) {
                this.$confirm('不选择收藏夹，将取消收藏该简历', '确认', {
                    ...
                    axiosGet(api.candidate + "UpdateUserFollow", { userId: this.userId, folderIds: this.userFolders.join(',') }).then(res => {
        ...
        delProfile: function (id) {
            this.$confirm('确定从所有收藏夹中删除？', '提示').then(active => {
                axiosGet(api.candidate + "UpdateUserFollow", { userId: id, folderIds: '' }).then(res => {
