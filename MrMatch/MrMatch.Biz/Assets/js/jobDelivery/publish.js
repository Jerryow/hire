...
window.vm = new Vue({
    ...
    data: {
        ...
        rules: {
            JobCompanyID: [{ required: true, message: '请选择企业', trigger: 'change' }],
            basicName: [{ required: true, message: '请选择职能', trigger: 'blur' }],
            JobName: [{ required: true, message: '请输入职位名称', trigger: 'change' }],
            funcListDefault: [{ required: true, message: '请选择职能', trigger: 'change' }],
            distListDefault: [{ required: true, message: '请选择地点', trigger: 'change' }],
            salaryOn: [{ required: true, message: '请输入年薪范围', trigger: 'change' }],
            salaryOff: [{ required: true, message: '请输入年薪范围', trigger: 'change' }],
            salaryOpen: [{ required: true, message: '请选择薪资是否公开', trigger: 'change' }],
            WorkYears: [{ required: true, message: '请输入工作年限', trigger: 'change' }],
            Degree: [{ required: true, message: '请选择学历', trigger: 'change' }],
            JobDescription: [{ required: true, message: '请输入职位介绍', trigger: 'change' }],
            JobRequirements: [{ required: true, message: '请输入岗位要求', trigger: 'change' }]
        },
        basicData: {
            agent: false,
            basicID: 0,
        ...
        educationType: [{ label: "不限", value: 0 }, { label: "大专", value: 40 }, { label: "本科", value: 50 }, { label: "硕士", value: 60 }, { label: "MBA/EMBA", value: 70 }, { label: "博士", value: 80 }],
        composition: {
            salaryComposition: [],
    ...
    methods: {
        getBasic: function () {
            return axiosGet(api.delivery + "GetBasicInfo").then(res => {
        ...
        getBasicFunc: function () {
            return axiosGet(api.candidate + "GetFunctionForCascader").then(res => {
                this.basicData.funcList = res.data;
            });
        },
        getBasicDist: function () {
            return axiosGet(api.candidate + "GetDistrictForCascader").then(res => {
        ...
        getJobPushProfile() {
            ...
            axiosGet(api.delivery + url, params).then(res => {
                if (res.code == 1) {
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    if (type == 'publish') {
                        ...
                        axiosPost(api.delivery + "SaveJob", JobForm).then(res => {
                    ...
                    if (type == 'draft') {
                        ...
                        axiosPost(api.delivery + "SaveJobDraft", DraftForm).then(res => {
                            this.$message(res.msg);
                            if (res.code == 1) {
        ...
        getSkillsList(id) {
            return axiosGet(api.delivery + "GetRelationSkills?PKID=" + id).then(res => {
                if (res.code == 1) {
                    let data = res.data;
