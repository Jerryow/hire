...
var vm = new Vue({
    ...
    data() {
        return {
            ...
            rules: {
                jobCode: [
                    { required: false, message: '请输入公司全称', trigger: 'change' }
                ],
                jobName: [
                    { required: false, message: '请输入公司全称', trigger: 'change' }
                ]
    ...
    methods: {
        ...
        getActivityJobList() {
            ...
            this.loading = true
            axiosGet(api.delivery + "GetActivityJobList", params)
                .then((res) => {
                    this.loading = false
                    if (res.code == 1) {
        ...
        getDraftJobList() {
            ...
            this.loading = true
            axiosGet(api.delivery + "GetJobDraftList", params)
                .then((res) => {
                    this.loading = false
                    if (res.code == 1) {
                        this.list = res.data ? res.data.DataList : []
        ...
        delDraftJob(item) {
            axiosGet(api.delivery + "DestroyJobDraft?PKID=" + item.PKID + "&accountID=" + item.AccountID)
                .then((res) => {
                    this.$message(res.msg)
                    if (res.code == 1) {
                        this.getDraftJobList()
        ...
        updateJobActivityStatus(item, type) {
            ...
            axiosGet(api.delivery + "UpdateActivityJobStatus", params)
                .then((res) => {
                    this.$message(res.msg)
                    if (res.code == 1) {
                        this.getActivityJobList()
