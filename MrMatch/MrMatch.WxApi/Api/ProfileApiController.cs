...
namespace MrMatch.WxApi.Api
{
    public class ProfileApiController : SecurityBaseController
    {
        ...
        public async Task<IHttpActionResult> GetUserInfo(long userID)
        {
            try
            {
                if (userID <= 0)
                ...
                var res = await userService.GetBasicUserAsync(userID);
        ...
        /// <returns></returns>
        [System.Web.Http.HttpGet]
        public async Task<IHttpActionResult> CommitApprove(long userID)
        {
            try
            {
                if (userID <= 0)
                ...
                //判断是否有更改
                var update = await userService.SnapUpdateStatusAsync(userID);
                if (!update.Education && !update.JobIntention && !update.Profile && !update.WorkEx)
                {
                    return Fail("您的简历没有任何改动,无需提交审核");
                }
                var res = await userService.CommitApproveAsync(userID);
                return Succcess("成功", res);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [System.Web.Http.HttpGet]
        public async Task<IHttpActionResult> UpdateUserActivity(long userID, bool status)
        {
            try
            {
                if (userID <= 0)
                ...
                var res = await userService.UpdateUserActivityAsync(userID, status);
                return Succcess("成功", res);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [System.Web.Http.HttpGet]
        public async Task<IHttpActionResult> SnapUpdateStatus(long userID)
        {
            try
            {
                var res = await userService.SnapUpdateStatusAsync(userID);
                return Succcess("成功", res);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [System.Web.Http.HttpGet]
        public async Task<IHttpActionResult> GetProfileSnap(long userID)
        {
            try
            {
                if (userID <= 0)
                ...
                var pro = await userService.GetProfileSnapAsync(userID);
                var edu = await userService.GetEducationSnapAsync(userID);
                var work = await userService.GetWorkExperienceSnapAsync(userID);
